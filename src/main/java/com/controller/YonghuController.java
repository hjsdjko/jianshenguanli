...
public class YonghuController {
    ...
    */
    ...
    public R xiaofei(@RequestParam Map<String, Object> params, HttpServletRequest request){
        logger.debug("xiaofei方法:,,Controller:{},,params:{}",this.getClass().getName(),JSONObject.toJSONString(params));
        String role = String.valueOf(request.getSession().getAttribute("role"));
        params.put("yonghuId",request.getSession().getAttribute("userId"));
        if("用户".equals(role)){
            return  R.error(511,"您没有权限购买月卡或年卡");
        }
        ...
        DictionaryEntity dictionaryEntity = dictionaryService.selectOne(new EntityWrapper<DictionaryEntity>().eq("dic_code", "kapian_types").eq("dic_name", "消费类型名称").eq("code_index", flag));
        BigDecimal xiaofeiMoney = new BigDecimal(dictionaryEntity.getBeizhu());



        if( (yonghuEntity.getNewMoney() - xiaofeiMoney.doubleValue()) <0 ){
            return  R.error(511,"用户余额不够消费"+xiaofeiMoney.doubleValue()+"金额，请让用户进行充值");
        }
        yonghuEntity.setNewMoney(yonghuEntity.getNewMoney() - xiaofeiMoney.doubleValue());//设置金额

        //如果会员截止时间小于当前时间 自动改为当前时间
        if(yonghuEntity.getYonghuTime().getTime() < new Date().getTime()){
            yonghuEntity.setYonghuTime(new Date());
        }
        yonghuEntity.setYonghuTypes(2);//状态改为会员
        SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd");
        Calendar  calendar = Calendar.getInstance();
        calendar.setTime(yonghuEntity.getYonghuTime());
//        String T3 = format.format(calendar.getTime() ) ;
        //加日期
        if(flag == 1){
            calendar.add(Calendar.DAY_OF_MONTH, 30);
//            yonghuEntity.setYonghuTime(new Date(yonghuEntity.getYonghuTime().getTime()+24*3600*1000*30));
        }else if(flag == 2){
            calendar.add(Calendar.DAY_OF_MONTH, 365);
        }else{
            return  R.error(511,"未知支付类型");
        }
        yonghuEntity.setYonghuTime(calendar.getTime());
...
//                            yonghuEntity.setYonghuTypes(Integer.valueOf(data.get(0)));   //状态 要改的
//                            yonghuEntity.setYonghuEmail(data.get(0));                    //电子邮箱 要改的
//                            yonghuEntity.setNewMoney(data.get(0));                    //余额 要改的
//                            yonghuEntity.setYonghuTime(new Date(data.get(0)));          //会员截止日期 要改的
